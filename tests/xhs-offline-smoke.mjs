...
assert.match(xhsOverrides, /--xhs-brand-clearance:\s*32px/, "小红书品牌标志应额外避开返回按钮区域");
assert.match(xhsOverrides, /\.is-xhs-tool \.app-header\s*\{[^}]*height:\s*calc\(72px \+ var\(--xhs-safe-top\) \+ var\(--xhs-brand-clearance\)\);[^}]*padding:\s*calc\(var\(--xhs-safe-top\) \+ var\(--xhs-brand-clearance\)\)/s, "小红书品牌标志应在安全区下方继续下移");
