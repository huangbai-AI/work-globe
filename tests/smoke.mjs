...
assert.match(styles, /@media \(max-width:\s*760px\)[\s\S]*?\.card-switch\s*\{[^}]*top:\s*50%;[^}]*width:\s*44px;[^}]*height:\s*44px;[^}]*translateY\(-50%\)/s, "移动端翻页键应位于卡片左右中线并满足 44px 触控尺寸");
assert.match(styles, /\.card-switch-prev\s*\{[^}]*left:\s*8px;[^}]*\}[\s\S]*?\.card-switch-next\s*\{[^}]*right:\s*8px;/s, "移动端翻页键应完整收进卡片左右边缘，避免被裁切");
assert.match(styles, /@media \(max-width:\s*760px\)[\s\S]*?\.job-card\s*\{[^}]*min-height:\s*258px;[^}]*padding:\s*18px 52px 16px;/s, "移动端岗位卡片应降低高度并为两侧翻页键留出内容安全距离");
assert.match(styles, /@media \(max-width:\s*760px\)[\s\S]*?\.job-card h1\s*\{[^}]*-webkit-line-clamp:\s*2;/s, "移动端岗位标题应最多显示两行并省略溢出内容");
assert.match(styles, /@media \(max-width:\s*760px\)[\s\S]*?\.job-card-facts\s*\{[^}]*flex-wrap:\s*nowrap;[^}]*overflow:\s*hidden;/s, "移动端地点与薪资应合并为单行");
assert.match(styles, /\.job-card-facts b\s*\{[^}]*text-overflow:\s*ellipsis;[^}]*white-space:\s*nowrap;/s, "移动端地点与薪资超长时应显示省略号");
assert.equal((html.match(/class="landing-title-line"/g) || []).length, 2, "首页标题应拆成两行依次浮现");
assert.match(html, /<span class="landing-title-line">全球工作<\/span>\s*<span class="landing-title-line">此刻近在眼前<\/span>/, "首页标题应使用确认后的两行无逗号文案");
...
assert.match(styles, /@media \(min-width:\s*981px\)[\s\S]*?\.landing-copy\s*\{[^}]*top:\s*48\.5%;/s, "桌面端标题与按钮组应进一步上移以平衡构图");
...
assert.match(html, /styles\.css\?v=20260722-smooth-web-motion/, "桌面转场调整后应更新样式缓存版本");
assert.match(html, /explore\.js\?v=20260722-smooth-web-motion/, "桌面镜头调整后应更新脚本缓存版本");
...
assert.match(exploreSource, /landingHomeView\s*=\s*Object\.freeze\(\{\s*lat:\s*42,\s*lng:\s*10\s*\}\)/, "首页默认视角应让北半球与欧洲面向观众");
assert.match(exploreSource, /width <= 760 \? Math\.round\(height \* 0\.43\)/, "移动端选中岗位后地球应进一步下移，避免与卡片重叠");
assert.match(exploreSource, /job\.lat >= 35 && job\.lat <= 60[\s\S]*?job\.lng >= -10 && job\.lng <= 30/, "首页进入探索时应优先打开欧洲城市岗位");
assert.match(exploreSource, /const recencyStart = new Date\(recencyEnd\)[\s\S]*?setUTCDate\(recencyStart\.getUTCDate\(\) - 29\)/, "网页端必须把岗位限制在最近 30 个自然日内");
assert.match(exploreSource, /function enterExplore\(\)[\s\S]*?selectJob\(entry\.job,/, "点击首页按钮后应直接进入岗位卡片状态");
assert.match(exploreSource, /function smootherStep\(progress\)[\s\S]*?progress \* progress \* progress/, "桌面地球镜头应使用首尾速度为零的平滑曲线");
assert.match(exploreSource, /function animateViewLayout\(layout, duration\)[\s\S]*?state\.globe\.globeOffset\(state\.offset\);[\s\S]*?state\.globe\.pointOfView\(\{[\s\S]*?\}, 0\);/s, "桌面地球偏移和镜头缩放应由同一逐帧动画统一驱动");
assert.match(exploreSource, /useUnifiedDesktopTransition[\s\S]*?skipCamera:\s*true,[\s\S]*?skipUrl:\s*true[\s\S]*?focusJob:\s*entry\.job/s, "桌面进入探索时不应连续启动两次镜头移动");
assert.match(styles, /@media \(min-width:\s*981px\)[\s\S]*?\.app-page\.is-explore\.is-transitioning \.job-card\s*\{[^}]*animation-delay:\s*380ms;/s, "桌面岗位卡片应在首页文字退场后再衔接出现");
assert.match(exploreSource, /els\.cardTitle\.title\s*=\s*job\.title;[\s\S]*?els\.cardLocation\.title\s*=\s*job\.location;[\s\S]*?els\.cardSalary\.title\s*=\s*job\.salary;/, "被省略的标题、地点和薪资应保留完整悬停说明");
...
const dom = new JSDOM(html, {
  ...
  url: "http://openwork.test/?view=explore&job=china-nvidia-jr2015614"
});
const { window } = dom;
...
assert.ok(window.WORK_JOBS.length > 500, "近 30 天刷新后应保留足够多的岗位");
assert.ok(window.WORK_JOBS.every((job) => {
  const postedDate = new Date(`${job.postedAt}T00:00:00Z`);
  return job.postedAt && postedDate >= recencyStart && postedDate <= recencyEnd;
}), "网站中展示的所有岗位都必须有日期且位于最近 30 天内");
