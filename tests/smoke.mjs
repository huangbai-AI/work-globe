...
assert.match(html, /explore\.js\?v=20260722-europe-home/, "欧洲默认视角调整后应使用新的缓存版本");
...
assert.equal(globeState.globeMaterial?.type, "MeshLambertMaterial", "海洋应使用可承接柔和光影的纯色材质");
assert.equal(globeState.globeMaterial?.color.getHexString(), "c4dae6", "海洋应使用与画布有清楚明度差的高明度雾蓝");
assert.ok(globeState.globeMaterial?.emissiveIntensity > 0, "海洋应保留轻微自发光，避免背光面变脏");
