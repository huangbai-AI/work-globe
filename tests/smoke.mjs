...
assert.match(styles, /\.card-switch-prev\s*\{[^}]*left:\s*-12px;[^}]*\}[\s\S]*?\.card-switch-next\s*\{[^}]*right:\s*-12px;/s, "移动端翻页键应跨在卡片左右边缘");
assert.match(styles, /@media \(max-width:\s*760px\)[\s\S]*?\.job-card\s*\{[^}]*padding:\s*21px 32px 20px;/s, "移动端岗位卡片应为两侧翻页键留出内容安全距离");
